...
out_str = "/extra2/knopf/vmodel_output/longsim_preyvis_astr/"
saveLoc = "/extra2/knopf/vmodel_output/data/"
saveName = "longsim_preyvis_astr_occ"
...
for i in range(len(paraChange1_val)):
    ...
    for j in range(len(paraChange2_val)):
        ...
        for rep in range(reps):
            ...
            com_pos_shift = com_pos[1:]-com_pos[:-1]
            mil_scan[i,j,rep,1:] = np.linalg.norm(com_pos_shift, axis = 1)
...
np.save(str(saveLoc)+""+str(saveName)+"_comdis_"+str(paraChange1_name)+"_"+str(paraChange2_name), mil_scan)
