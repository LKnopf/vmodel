...
out_str = "/extra2/knopf/vmodel_output/hm_millstart_fov360_occ/"
saveLoc = "/extra2/knopf/vmodel_output/data/"
saveName = "hm_millstart_fov360_occ"
...
IID_scan = np.zeros((steps, steps, reps))
CND_scan = np.zeros((steps, steps, reps))
maxDist_scan = np.zeros((steps, steps, reps))
NN_scan = np.zeros((steps, steps, reps))
            ...
            with h5py.File(file_h5) as fh5:
                ...
                for rep in range(reps):
                    ...
                    for ii in range(time):
                        ...
                        IID.append(np.mean(dist))
                        maxDist.append(np.max(dist))
                        
                        dist[dist==0]=100
                        CND.append(np.min(dist))
                    ...
                    NN_scan[i, j, rep] = np.mean(np.sum(vis_rep, axis = 1))
                    CND_scan[i,j, rep] = np.min(CND)
                    IID_scan[i,j, rep] = np.mean(IID)
                    maxDist_scan[i,j, rep] = np.max(maxDist)
                    ...
                    np.save(str(saveLoc)+""+str(saveName)+"_IID_"+str(paraChange1_name)+"_"+str(paraChange2_name), IID_scan)
                    np.save(str(saveLoc)+""+str(saveName)+"_maxSize_"+str(paraChange1_name)+"_"+str(paraChange2_name), maxDist_scan)
                    np.save(str(saveLoc)+""+str(saveName)+"_NN_"+str(paraChange1_name)+"_"+str(paraChange2_name), NN_scan)
