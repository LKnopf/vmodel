...
def theta(v, w): return 180*arccos(v.dot(w)/(norm(v)*norm(w)))/math.pi
...
out_str = "/extra2/knopf/vmodel_output/longsim_preyvis240_dphi_astr_vor_NoCol/"
saveLoc = "/extra2/knopf/vmodel_output/data/"
saveName = "longsim_preyvis240_dphi_astr_vor_Nocol"
        ...
        for rep in range(reps):
            ...
            for ii in range(time):
                ...
                for jj in range(nprey):
                    ...
                    angle_i = theta(dir_com[jj,:], vel_rep[ii,jj,:])
...
np.save(str(saveLoc)+""+str(saveName)+"_mill3_"+str(paraChange1_name)+"_"+str(paraChange2_name), mil_scan)
