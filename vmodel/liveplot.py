...
def plot_all(ax, data, args, focal=50):
...
def plot_nselect_metric(ax, positions, perception_radius, focal=50):
    x, y = positions[focal]
...
def plot_nselect_visual(ax, positions, radius=0.25, focal=50):
...
def plot_nselect_topo(ax, positions, visibility, focal=50):
    pos_self = positions[focal]
    x, y = pos_self
...
def plot_agents(ax, positions, velocities=None, visibility=None, radius=0.25,
                focal_agent=50, show_identity=False, tail_length=0):
    """Plot agents
    Args:
    ...
    for a in range(len(post)):
        ...
        # Plot visibilitypred_hunt = False
